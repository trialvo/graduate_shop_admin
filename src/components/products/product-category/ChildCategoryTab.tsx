...
export default function ChildCategoryTab() {
  const [mainCategories] = useState<CategoryRow[]>(INITIAL_CATEGORIES);
  const [subCategories] = useState<SubCategoryRow[]>(INITIAL_SUB_CATEGORIES);
  const [rows, setRows] = useState<ChildCategoryRow[]>(INITIAL_CHILD_CATEGORIES);
  ...
  const availableSubCategories = useMemo(() => {
    if (mainCategoryId === null) return subCategories;
    return subCategories.filter((s) => s.mainCategoryId === mainCategoryId);
  }, [subCategories, mainCategoryId]);

  const subCategoryOptions: Option[] = useMemo(() => {
    return availableSubCategories.map((s) => ({ value: String(s.id), label: s.name }));
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      const sc = subCategories.find((s) => s.id === r.subCategoryId)?.name ?? "";
      return (
        r.name.toLowerCase().includes(q) ||
        mc.toLowerCase().includes(q) ||
        sc.toLowerCase().includes(q)
      );
    });
  }, [rows, search, mainCategories, subCategories]);
  ...
  const addRow = () => {
    ...
    if (!trimmed || mainCategoryId === null || subCategoryId === null) return;
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-[4px] border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
        ...
        <div className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              placeholder={
                mainCategoryId === null ? "Select Main Category First" : "Select Sub Category"
              }
              onChange={(v) => setSubCategoryId(Number(v))}
              defaultValue={subCategoryId !== null ? String(subCategoryId) : ""}
            />
          </div>
        ...
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          ...
          <Button
            onClick={addRow}
            disabled={!name.trim() || mainCategoryId === null || subCategoryId === null}
          >
            Add
          </Button>
        </div>
        ...
        <div className="flex flex-col gap-3 border-b border-gray-200 p-4 dark:border-gray-800 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center md:w-auto">
            <div className="relative w-full sm:w-[280px]">
              ...
              <Input
                className="pl-9"
                placeholder="Ex : search child categories"
                value={search}
            ...
            <Button
              ...
              onClick={() => {
                ...
                console.log("Export child categories");
              }}
            >
              Export
            </Button>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          onClick={() => {
                            // eslint-disable-next-line no-console
                            console.log("Edit child category", row.id);
                          }}
                          aria-label="Edit"
                        >
                          <Pencil size={16} />
